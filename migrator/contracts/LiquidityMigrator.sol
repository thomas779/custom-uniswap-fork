...
contract LiquidityMigrator {
    ...
    function deposit(uint amount) external {
        require(migrationDone == false, 'migration already completed');
    ...
    function migrate() external {
        require(admin == msg.sender, 'only powerful people can access this');
        require(migrationDone == false, 'migration has been compeleted');
    ...
    function claimLpTokens() external {
        require(unclaimedBalances[msg.sender] >= 0, 'no balance to claim');
        require(migrationDone == true, 'migration uncompleted');
